...
public class WebConfig implements WebMvcConfigurer {
    ...
    // ✅ 기본 페이지 설정 (캘린더 및 newTradeCompany)
    @Override
    public void addViewControllers(ViewControllerRegistry registry) {
        ...
        registry.addViewController("/newTradeCompany").setViewName("newTradeCompany"); // ✅ 수정됨
    }

    // ✅ JSON 변환기 설정 추가 (MappingJackson2HttpMessageConverter)
    @Override
    ...
    public WebMvcConfigurer corsConfigurer() {
        return new WebMvcConfigurer() {
            ...
            public void addCorsMappings(CorsRegistry registry) {
                registry.addMapping("/**")
                        .allowedOriginPatterns("http://localhost:80") // 모든 출처 허용
